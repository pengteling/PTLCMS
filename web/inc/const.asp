...
%>
       <li class="newslist"><div class="con"><span class="time"><%=formatdate(NewsListRs("posttime"),2)%></span><a href="<%=infoURL(NewsListRs("staticUrl"),NewsListRs("dynamicUrl"),supportHtml)%>"><%=leftstr(NewsListRs("title"),90)%></a>
       </div>
       </li>       
<%		
...
Function ShowNews()
	...
	cateid = strToNum(Request("id"))	
	Keyword = ChkFormStr(Request("so"))
	msql="Select id,title,posttime,content_zy,comments,px,defaultpicurl,content,hits,staticUrl,dynamicUrl from news_c where 1=1 "
	If cateid<>0 Then
		'msql=msql&" and cateid="&cateid&""
		msql=msql&" and cateid in ("&sonid&")"
	End If
...
%>
       ...
       <li>
              <div class="pic"><a href="readnews.asp?id=<%=NewsListRs("id")%>"><img src="<%=NewsListRs("defaultpicurl")%>" alt=""></a></div>
              <div class="fr">
                <div class="tit"><a href="readnews.asp?id=<%=NewsListRs("id")%>" title="<%=NewsListRs("title")%>"><%=leftstr(NewsListRs("title"),44)%></a></div>
                <div class="intro"><%=leftstr(NewsListRs("content_zy"),190)%></div>
                <div class="info">
                  <div class="time"><%=formatdate(NewsListRs("posttime"),2)%></div>
                  <div class="hits"><%=NewsListRs("hits")%></div>
                  <div class="comments"><%if(isnull(NewsListRs("comments"))) then response.write "0" else response.write NewsListRs("comments") end if%></div>
                </div>
              </div>
            </li>
...
Function ShowSearch()
	...
	If Keyword<>"" Then
		msql=msql&" and (title like '%"&Keyword&"%' or content like '%"&Keyword&"%' or keys like  '%"&Keyword&"%' )"
	End If
...
%>
       <li>
              <div class="pic"><a href="readnews.asp?id=<%=NewsListRs("id")%>"><img src="<%=NewsListRs("defaultpicurl")%>" alt=""></a></div>
              <div class="fr">
                <div class="tit"><a href="readnews.asp?id=<%=NewsListRs("id")%>"><%=replace(leftstr(NewsListRs("title"),44),Keyword,"<span class='search-red'>"&Keyword&"</span>")%></a></div>
                <div class="intro"><%=replace(leftstr(NewsListRs("content_zy"),190),Keyword,"<span class='search-red'>"&Keyword&"</span>")%></div>
                <div class="info">
                  <div class="time"><%=formatdate(NewsListRs("posttime"),2)%></div>
                  <div class="hits"><%=NewsListRs("hits")%></div>
                  <div class="comments"><%if(isnull(NewsListRs("comments"))) then response.write "0" else response.write NewsListRs("comments") end if%></div>
...
Function ShowPic()
	...
	cateid = strToNum(Request("id"))	
	Keyword = ChkFormStr(Request("so"))
	msql="Select id,title,posttime,content_zy,comefrom,comments,px,defaultpicurl,content,hits,staticUrl,dynamicUrl from news_c where 1=1 "
	If cateid<>0 Then
		'msql=msql&" and cateid="&cateid&""
		msql=msql&" and cateid in ("&sonid&")"
	End If
...
<li><a href="readnews.asp?id=<%=NewsListRs("id")%>">
							<img src="<%=NewsListRs("defaultpicurl")%>" alt="">
							<div class="greymb"><h3 class="ptit"><%=leftstr(NewsListRs("title"),90)%></h3></div>
							<div class="bluemb">
								<h3><%=leftstr(NewsListRs("title"),90)%></h3>
								<p><%=leftstr(NewsListRs("content_zy"),290)%></p>
							</div>
							<div class="pnum">· <%=leftstr(NewsListRs("comefrom"),290)%> ·</div>
							</a>
...
%>
       ...
       <a href="<%=infoURL(NewsListRs("staticUrl"),NewsListRs("dynamicUrl"),supportHtml)%>"><img src="<%=NewsListRs("defaultpicurl")%>" /></a>
              ...
              <p><a href="<%=infoURL(NewsListRs("staticUrl"),NewsListRs("dynamicUrl"),supportHtml)%>"><%=NewsListRs("title")%></a></p>
            </li>
